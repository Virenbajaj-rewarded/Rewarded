...
export const QRScanner: React.FC = () => {
  ...
  const mapRecent = (
    ...
    items.map(it => ({
      ...
      action: it.points >= 0 ? 'Earned' : 'Redeemed',
  ...
  const addEventTop = (action: 'Earned' | 'Redeemed', points: number) => {
    setRecentScans(prev => [
      { customer: phone || 'Unknown', action, points, time: 'Just now' },
      ...prev.slice(0, 2),
    ]);
  };
  ...
  const handleEarn = async () => {
    ...
    try {
      ...
      const { data } = await api.post<{ points: number }>(
        '/ledger/earn',
        payload
      );
      toast({ title: 'Points Issued', description: `+${data.points} pts` });
      addEventTop('Earned', data.points);
      setPurchaseAmount('');
      loadRecent();
    } catch (err: any) {
      const msg =
  ...
  const handleRedeem = async () => {
    ...
    try {
      ...
      addEventTop('Redeemed', data.points);
      setRedeemPoints('');
      loadRecent();
    } catch (err: any) {
      const msg =
              ...
              <div className="aspect-square bg-muted/20 border-2 border-dashed border-border rounded-lg flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-muted-foreground mb-4">
                    Position customer QR code in view
                  </p>
            ...
            {recentScans.map((scan, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <Badge
                    variant={
                      scan.action === 'Earned' ? 'default' : 'destructive'
                    }
                  >
                    {scan.action}
                  </Badge>
