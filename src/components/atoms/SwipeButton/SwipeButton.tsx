...
const SliderButton = ({ disabled, buttonWidth, onConfirm }: SliderButtonProperties) => {
  const position = useSharedValue(PADDING);
  const completed = useSharedValue(false);

  const END_POSITION = useMemo(() => buttonWidth - BUTTON_SIZE - PADDING, [buttonWidth]);

  const panGesture = Gesture.Pan()
    .enabled(!disabled)
    .onUpdate(e => {
      position.value = Math.min(
        Math.max(PADDING, e.translationX + (completed.value ? END_POSITION : PADDING)),
        END_POSITION
      );
    })
    .onEnd(() => {
  ...
  return (
    <Animated.View style={[styles.sliderContainer, { width: buttonWidth }, animatedContainerStyle]}>
      <Animated.Text style={[styles.sliderText, animatedTextStyle]}>Confirm</Animated.Text>
      <Animated.Text style={[styles.sliderText, animatedCompletedTextStyle]}>Done</Animated.Text>
