...
import { StoreServices } from "@/hooks/domain/stores/storesService.ts";
...
export const useDeleteStore = () => {
  ...
  return useMutation({
    ...
    onSuccess: (id) => {
      client.setQueryData<
        { pages: StoresResponse[]; pageParams: unknown[] } | undefined
      >([StoreQueryKey.fetchMyStores], (oldData) => {
        if (!oldData) {
          return oldData;
        }
...
export const useRestoreStore = () => {
  ...
  return useMutation({
    ...
    onSuccess: (id) => {
      client.setQueryData<
        { pages: StoresResponse[]; pageParams: unknown[] } | undefined
      >([StoreQueryKey.fetchDeletedStores], (oldData) => {
        if (!oldData) {
          return oldData;
        }
